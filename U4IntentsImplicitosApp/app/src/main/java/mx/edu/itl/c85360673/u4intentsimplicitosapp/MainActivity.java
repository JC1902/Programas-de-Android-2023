...
public class MainActivity extends AppCompatActivity {
    ...
    public static final int CODIGO_ARCHIVO_MAIL         = 1946;
    ...
    public void  btnAbrirPaginaWebClick    ( View v ) {
        layout      = getLayoutInflater().inflate ( R.layout.layout_edit_text_simple, null );
        edtTexto  = layout.findViewById  ( R.id.edtTexto );
        edtTexto.setText ( "https://pilaweb.000webhostapp.com/index.html" );
    ...
    public void btnEnviarCorreoClick      ( View v ) {
        layout    = getLayoutInflater().inflate ( R.layout.layout_datos_mail, null );
        edtDestino = layout.findViewById( R.id.edtDestino );
        edtCC = layout.findViewById( R.id.edtCC );
        edtTexto  = layout.findViewById  ( R.id.edtMensaje );
        edtAsunto = layout.findViewById( R.id.edtAsunto );

        edtTexto.setText ( texto );
        edtCC.setText( texto );
        edtAsunto.setText( texto );
        edtDestino.setText( texto );

        AlertDialog.Builder builder = new AlertDialog.Builder ( this );
        builder.setTitle ( "Ingrese los datos del email:" )
                .setView ( layout )
    ...
    //----------------------------------------------------------------------------------------------
    // Enviar correo electronico simple ( con adjuntos )
    //                                  ACTION_SEND
    // Para lograr esto se separo el método el primero el abrirExploradorArchivos() se llama ya que
    // el usuario vacío los datos y presiono enivar, en este se encuentra un startActivityForResult
    // con el código para ejecutar un método que recibe el uri y los datos que el usuario vacio y
    // se mandan a la app de correo que el usuario seleccione

    public void btnEnviarCorreoComplejoClick    ( View v ) {
        layout = getLayoutInflater().inflate(R.layout.layout_datos_mail, null);
        edtDestino = layout.findViewById(R.id.edtDestino);
        edtCC = layout.findViewById(R.id.edtCC);
        edtTexto = layout.findViewById(R.id.edtMensaje);
    ...
    protected void onActivityResult ( int requestCode, int resultCode, @Nullable Intent data) {
        ...
        } else if ( requestCode == CODIGO_ARCHIVO_MAIL ) {
            if ( resultCode == RESULT_OK ) {
                if (data != null) {
                    selectedFileUri = data.getData();
                    enviarCorreoConArchivoAdjunto(selectedFileUri);
                }
            }
        }
    }
    ...
    //----------------------------------------------------------------------------------------------
    // Abrir el explorador de archivos con el codigo para enviar un archivo por email

    public void abrirExploradorArchivos() {
        Intent intent = new Intent( Intent.ACTION_GET_CONTENT );
        intent.setType( "*/*" );
        startActivityForResult(intent, CODIGO_ARCHIVO_MAIL );
    }

    //----------------------------------------------------------------------------------------------
    // Como con el mail simple obtenemos todos los datos y para el archivo adjunto usamos
    // EXTRA_STREAM ademas modificamos para el setType para que no solo sea texto plano

    public void enviarCorreoConArchivoAdjunto    (Uri archivoAdjunto) {
        String[] destino = { edtDestino.getText().toString() };
        String[] cc = { edtCC.getText().toString() };
        String asunto = edtAsunto.getText().toString();
        String mensaje = edtTexto.getText().toString();
        ...
        intent.putExtra( Intent.EXTRA_SUBJECT, asunto );
        intent.putExtra( Intent.EXTRA_TEXT, mensaje );
        intent.putExtra( Intent.EXTRA_STREAM, archivoAdjunto );
